...
- **Offline Reader View.** Full-text extraction runs entirely on-device with Mozilla Readability — no API keys and no hosted parser, so Reader View works in any build. It strips page chrome (consent banners, navigation, modals, in-body ads and newsletter promos), has per-site rules for tricky sites (including reassembling articles that publishers split across multiple pages, such as Ars Technica features), and reuses the app's logged-in session so it can pull full articles from sites you're signed into. Pinch open over an article to enter Reader View, pinch closed to leave it.
...
- **Sidebar read filter.** An Unread / All segmented control pinned to the bottom of the sidebar quickly toggles between showing every feed and hiding feeds that have no unread articles. The sidebar and timeline read filters stay in sync, so flipping one updates the other.
- **Mark-all-as-read confirmation.** Marking everything as read no longer happens instantly. A small confirmation toast slides down over the timeline with Cancel / Mark All as Read buttons, so an accidental shortcut never wipes the list. It's shared by both the sidebar and timeline actions, dismisses on an outside click, and adapts to light and dark mode.
